...
{
    public static class GUIPaginationUtilities
    {
        public static void DrawPagesWidget(int itemsCount, PaginationSettings settings)
        {
            if (itemsCount == 0 || settings.PageSize == 0)
                return;
            
            var totalCount = itemsCount;
            var pagesCount = totalCount / settings.PageSize + (totalCount % settings.PageSize > 0 ? 1 : 0);
            ...
            for (var i = 0; i < pagesCount; i++)
            {
                ...
                if (pagesCount > 100)
                {
                    ...
                    if (i != 0 && i != pagesCount - 1 && i % 100 != 0 && (i < anchorPage - 20 || i > anchorPage + 20))
                        shouldDraw = false;
                }
                ...
                if (shouldDraw)
                {
                    ...
                    if (GUILayout.Button((i + 1).ToString(), GUILayout.Width(i <= 998 ? 30f : 60f)))
                    {
        ...
        public static bool ShouldDrawItem(int index, PaginationSettings settings)
        {
            if (settings.PageToShow.HasValue)
            {
